...
net_w = 416
# 检测模型的类别
# class_names = ["yellow_back"]
class_names = ["pedestrian_crossing", 'speed_limited', 'speed_unlimited', 'traffic_light']
_class_names = {0: 0,1:1,2:2,3:3}
...
def get_boxes(bboxes,light):
    """
    get bboxes if pedestrian crossing exists and output the bbox
    :param bboxes: detected object bbox
    :return: list
    """
    bbox_list = []
    bbox_info = {}
    for bbox in bboxes:
        ...
        if score >= 0.80:
            label = _class_names[int(bbox[4])]
            bbox_info["label"] = class_names[label]
            # get the pedestrian crossing and yellow light box
            bbox_info["x_min"] = int(bbox[0])
            bbox_info["y_min"] = int(bbox[1])
            bbox_info["x_max"] = int(bbox[2])
            bbox_info["y_max"] = int(bbox[3])
            if int(bbox[4]) == 3:
                bbox_info["light"] = traffic_lights[int(light)]
...
def socketSendMsg(socketObj, info):
    info = str(info)
    try:
        connection, address = socketObj.accept()  # Socket Accept
        #connection.send(info.encode("utf-8"))  # Socket Send
        # print("Sent: ", info)
        connection.close()
    except:
        pass
